...
@testsuite :tensors "tensor constructions" V -> begin
...
@testsuite :tensors "contractions" V -> begin
    I = sectortype(first(V))
...
@testsuite :tensors "linear algebra" V -> begin
    I = sectortype(first(V))
    V1, V2, V3, V4, V5 = V
...
@testsuite :tensors "index manipulations" V -> begin
    I = sectortype(first(V))
...
@testsuite :tensors "braiding tensor" V -> begin
    I = sectortype(first(V))
    BraidingStyle(I) isa NoBraiding && return nothing
    Vspace = first(V)
    T = ComplexF64
    t = randn(T, Vspace ⊗ Vspace' ⊗ Vspace' ⊗ Vspace ← Vspace ⊗ Vspace')
