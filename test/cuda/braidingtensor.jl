...
for V in (V1, V2, V3)
    t = randn(CuVector{ComplexF64}, V * V' * V' * V, V * V')

    ττ = TensorMap(BraidingTensor(V, V', CuVector{ComplexF64}))
    @test TensorKit.storagetype(ττ) == CuVector{ComplexF64, CUDA.DeviceMemory}
