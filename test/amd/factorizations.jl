...
for V in spacelist
    ...
    @timedtestset "Factorizations with symmetry: $Istr" verbose = true begin
        ...
        @testset "QR decomposition" begin
            for T in eltypes,
                    t in (
                        AMDGPU.rand(T, W, W), AMDGPU.rand(T, W, W)',
                        AMDGPU.rand(T, (V1 ⊗ V2 ⊗ V3), (V4 ⊗ V5)'), AMDGPU.rand(T, (V1 ⊗ V2 ⊗ V3), (V4 ⊗ V5)')',
                        AMDGPU.rand(T, (V1 ⊗ V2)', (V3 ⊗ V4 ⊗ V5)), AMDGPU.rand(T, (V1 ⊗ V2)', (V3 ⊗ V4 ⊗ V5))',
                        DiagonalTensorMap(AMDGPU.rand(T, reduceddim(V1)), V1),
                    )
            ...
            # empty tensor
            for T in eltypes
                t = AMDGPU.rand(T, V1 ⊗ V2, zerospace(V1))
        ...
        @testset "LQ decomposition" begin
            ...
            for T in eltypes
                # empty tensor
                t = AMDGPU.rand(T, zerospace(V1), V1 ⊗ V2)
        ...
        @testset "SVD" begin
            ...
            for T in eltypes, t in (
                        AMDGPU.rand(T, W, zerospace(V1)),
                        AMDGPU.rand(T, zerospace(V1), W),
                    )
                U, S, Vᴴ = @constinferred svd_full(t)
                @test U * S * Vᴴ ≈ t
                @test isunitary(U)
        ...
        @testset "truncated SVD" begin
            for T in eltypes,
                    t in (
                        AMDGPU.randn(T, W, W), AMDGPU.randn(T, W, W)',
                        AMDGPU.randn(T, (V1 ⊗ V2 ⊗ V3), (V4 ⊗ V5)'), AMDGPU.randn(T, (V1 ⊗ V2)', (V3 ⊗ V4 ⊗ V5))',
                        AMDGPU.randn(T, (V1 ⊗ V2), (V3 ⊗ V4 ⊗ V5)'), AMDGPU.randn(T, (V1 ⊗ V2 ⊗ V3)', (V4 ⊗ V5))',
                        #DiagonalTensorMap(AMDGPU.randn(T, reduceddim(V1)), V1),
                    )
        ...
        @testset "Eigenvalue decomposition" begin
            for T in eltypes,
                ...
                # ROCSOLVER does not provide geev
        ...
        @testset "Condition number and rank" begin
            ...
            for T in eltypes
                ...
                t = AMDGPU.rand(T, zerospace(V1), W)
                @test rank(t) == 0
                t2 = AMDGPU.rand(T, zerospace(V1) * zerospace(V2), zerospace(V1) * zerospace(V2))
                @test rank(t2) == 0
                @test cond(t2) == 0.0
            end
            #=for T in eltypes, t in (
            ...
            end=# # ROCSOLVER doesn't provide geev
        end
