    ...
    @timedtestset "Tensors with symmetry: $Istr" verbose = true begin
        ...
        @timedtestset "Basic linear algebra" begin
            ...
            for T in (Float32, ComplexF64)
                ...
                i1 = @constinferred(isomorphism(ROCMatrix{T}, V1 ⊗ V2, V2 ⊗ V1))
                i2 = @constinferred(isomorphism(ROCMatrix{T}, V2 ⊗ V1, V1 ⊗ V2))
                @test i1 * i2 == @constinferred(id(ROCMatrix{T}, V1 ⊗ V2))
                @test i2 * i1 == @constinferred(id(ROCMatrix{T}, V2 ⊗ V1))

                w = @constinferred(isometry(ROCMatrix{T}, V1 ⊗ (oneunit(V1) ⊕ oneunit(V1)),
                                            V1))
                @test dim(w) == 2 * dim(V1 ← V1)
                @test w' * w == id(ROCMatrix{T}, V1)
                @test w * w' == (w * w')^2
            end
        end
        if hasfusiontensor(I)
        ...
        #=@timedtestset "Trace and contraction" begin
            ...
            @test ta ≈ tb
        end=# # Broken currently due to scalar indexing
        if BraidingStyle(I) isa Bosonic && hasfusiontensor(I)
